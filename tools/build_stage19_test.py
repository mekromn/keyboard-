#!/usr/bin/env python3
"""Rebuild a Stage-19 TEST from the exact verified Stage-18 APK.
...
from remove_base_clearcut_stage19 import apply
from verify_base_clearcut_stage19 import verify

INPUT_SHA='29b41378b2cec4ec805abf9a9dc0fdbdbb8a265d1d089c4b7aa888de896c9ba8'
...
def main():
    ...
    require(digest(args.input)==INPUT_SHA,'Input APK is not the reviewed Stage-18 checkpoint')
    ...
    run(['java','-Xmx3g','-jar',args.apktool,'d','-f','-j','4','-o',baseline,args.input],'decode.log')
    ...
    dexes={}
    for directory,name in [('smali','classes.dex'),('smali_classes2','classes2.dex')]:
        output=work/name
        run(['java','-Xmx3g','-cp',str(args.apktool)+os.pathsep+str(compiler),
        ...
        dexes[name]=data
    unsigned=work/'stage19-unsigned.apk'
    ...
    certificate=None
    if args.keystore:
        aligned=work/'stage19-aligned.apk'
    ...
    with zipfile.ZipFile(args.input) as before,zipfile.ZipFile(args.output) as after:
        ...
        require(changed==['classes.dex','classes2.dex'],'Unexpected non-code payload delta')
