#!/usr/bin/env python3
"""Rebuild a Stage-24 TEST from the exact verified Stage-23 APK.
...
from remove_provider_stats_stage24 import apply
from verify_provider_stats_stage24 import verify
...
def main():
    ...
    require(digest(args.input)==INPUT_SHA,'Input APK is not the reviewed Stage-23 checkpoint')
    ...
    for directory,name in [('smali','classes.dex'),('smali_classes2','classes2.dex')]:
        ...
        dexes[name]=data
    unsigned=work/'stage24-unsigned.apk'
    ...
    certificate=None
    if args.keystore:
        aligned=work/'stage24-aligned.apk'
