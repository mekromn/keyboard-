#!/usr/bin/env python3
"""Rebuild a Stage-20 TEST from the exact verified Stage-19 APK.
...
from remove_trainer_residue_stage20 import apply
from verify_trainer_residue_stage20 import verify
...
def main():
    ...
    require(digest(args.input)==INPUT_SHA,'Input APK is not the reviewed Stage-19 checkpoint')
    ...
    for directory,name in [('smali','classes.dex'),('smali_classes2','classes2.dex')]:
        ...
        dexes[name]=data
    unsigned=work/'stage20-unsigned.apk'
    ...
    certificate=None
    if args.keystore:
        aligned=work/'stage20-aligned.apk'
