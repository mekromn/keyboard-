#!/usr/bin/env python3
"""Rebuild a Stage-21 TEST from the exact verified Stage-20 APK.
...
from remove_federated_rescheduler_stage21 import apply
from verify_federated_rescheduler_stage21 import verify
...
def main():
    ...
    require(digest(args.input)==INPUT_SHA,'Input APK is not the reviewed Stage-20 checkpoint')
    ...
    for directory,name in [('smali','classes.dex'),('smali_classes2','classes2.dex')]:
        ...
        dexes[name]=data
    unsigned=work/'stage21-unsigned.apk'
    ...
    certificate=None
    if args.keystore:
        aligned=work/'stage21-aligned.apk'
