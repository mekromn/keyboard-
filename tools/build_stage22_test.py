#!/usr/bin/env python3
"""Rebuild a Stage-22 TEST from the exact verified Stage-21 APK.
...
from remove_initial_federated_stage22 import apply
from verify_initial_federated_stage22 import verify
...
def main():
    ...
    require(digest(args.input)==INPUT_SHA,'Input APK is not the reviewed Stage-21 checkpoint')
    ...
    for directory,name in [('smali','classes.dex'),('smali_classes2','classes2.dex')]:
        ...
        dexes[name]=data
    unsigned=work/'stage22-unsigned.apk'
    ...
    certificate=None
    if args.keystore:
        aligned=work/'stage22-aligned.apk'
