#!/usr/bin/env python3
"""Rebuild a Stage-26 TEST from the exact verified Stage-25 APK.
...
from remove_primes_battery_stage26 import apply
from verify_primes_battery_stage26 import verify
...
def main():
    ...
    require(digest(args.input)==INPUT_SHA,'Input APK is not the reviewed Stage-25 checkpoint')
    ...
    for directory,name in [('smali','classes.dex'),('smali_classes3','classes3.dex')]:
        ...
        dexes[name]=data
    unsigned=work/'stage26-unsigned.apk'
    ...
    certificate=None
    if args.keystore:
        aligned=work/'stage26-aligned.apk'
