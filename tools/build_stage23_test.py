#!/usr/bin/env python3
"""Rebuild a Stage-23 TEST from the exact verified Stage-22 APK.
...
from remove_federated_timing_stage23 import apply
from verify_federated_timing_stage23 import verify
...
def main():
    ...
    require(digest(args.input)==INPUT_SHA,'Input APK is not the reviewed Stage-22 checkpoint')
    ...
    for directory,name in [('smali','classes.dex'),('smali_classes2','classes2.dex')]:
        ...
        dexes[name]=data
    unsigned=work/'stage23-unsigned.apk'
    ...
    certificate=None
    if args.keystore:
        aligned=work/'stage23-aligned.apk'
